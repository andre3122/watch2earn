...
function getOrCreateUserFromTG(user, referred_by_code=null) {
  ...
  if (!row) {
    ...
    db.prepare('UPDATE users SET username=?, first_name=? WHERE tg_id=?').run(user.username||null, user.first_name||null, user.id)
  }
  return row
}
...
// ==== Bot ====
import { Telegraf as TG } from 'telegraf' // alias not necessary; keep original
...
app.post('/api/config', (req, res) => {
  ...
  res.json({ ok:true, vastTag: VAST_TAG, reward: REWARD_PER_TASK, min_withdraw: MIN_WITHDRAW, ref_bonus_pct: REF_BONUS_PCT, balance: me.balance, total_tasks: me.total_tasks })
})
...
app.post('/api/task/start', (req, res) => {
  ...
  // client will load VAST via Fluid Player; fallback min seconds if ad blocked
  res.json({ ok:true, task_id: taskId, min_watch_sec: 15 })
})

// Complete (called after player fires adEnded or fallback timer)
app.post('/api/task/complete', (req, res) => {
  ...
  db.prepare('UPDATE tasks SET status=?, completed_at=datetime(\'now\') WHERE id=?').run('completed', t.id)
...
app.post('/api/withdraw', (req, res) => {
  ...
  if (row.balance < MIN_WITHDRAW) return res.status(400).json({ ok:false, error:'min withdraw not met', balance: row.balance })
